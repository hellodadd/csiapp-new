...
class InMemoryTreeNode<T> implements Serializable {
    ...
    public InMemoryTreeNode(final T id, final T parent, final int level, final String description,
    ...
    public synchronized InMemoryTreeNode<T> add(final int index, final T child,
            final boolean visible, String description) {
        ...
        final InMemoryTreeNode<T> newNode = new InMemoryTreeNode<T>(child,
                getId(), getLevel() + 1, description, getId() == null ? true : visible);
