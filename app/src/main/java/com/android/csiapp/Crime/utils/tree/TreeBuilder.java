...
public class TreeBuilder<T> {
     ...
     * {@link } and then after the right level is added as parent,
     ...
     *            its level
     */
    public synchronized void sequentiallyAddNextNode(final T id, final int level, String description) {
        //Log.d(TAG, "Adding sequentiall node with description" + id + " at level " + level);
        if (lastAddedId == null) {
            addNodeToParentOneLevelDown(null, id, level, description);
        } else {
            if (level <= lastLevel) {
                final T parent = findParentAtLevel(lastAddedId, level - 1);
                addNodeToParentOneLevelDown(parent, id, level, description);
            } else {
                addNodeToParentOneLevelDown(lastAddedId, id, level, description);
            }
        }
    }
    ...
    private void addNodeToParentOneLevelDown(final T parent, final T id,
            final int level, String description) {
        ...
        manager.addAfterChild(parent, id, null, description);
